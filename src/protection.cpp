...
void Protection::update() {
  ...
  //run through second pack
  for (uint8_t i = 17; i < NUM_CELLS; i++) {
    uint16_t voltage = _measurements->cell_voltage(i);
    sum_voltage1 += voltage;
  ...
  if (_ov1 && max_voltage < OV_ENABLE && max_voltage < OV_DISABLE) {
    _ov1 = false;
  }
  ...
  if (abs((int16_t) sum_voltage1 - (int16_t) _measurements->pack1_voltage()) >= MAX_VOLTAGE_ERROR) {
    // we've exceeded max allowable error in voltage measurements
    _fault = true;
  }
}
